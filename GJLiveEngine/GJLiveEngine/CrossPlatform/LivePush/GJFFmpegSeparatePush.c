...
static GHandle sendRunloop(GHandle parm) {
    ...
    if (push->videoFormat) {
        GLong index = 0;
        while (queuePeekWaitValue(push->videoQueue, index++, (GHandle *) &packet, GINT32_MAX)) { //过滤无效起始数据
            if (push->stopRequest) {
                goto END;
            }
            ...
            if (packet->type == GJMediaType_Video) {
                ...
                if (packet->extendDataSize < 0 || (start[4] & 0x1f) != 7) {
                    ...
                    queuePop(push->videoQueue, (GHandle) packet, 0);
    ...
    while (!push->stopRequest) {
        packet = GNULL;
        while (!queuePop(push->audioQueue, (GHandle *) &packet, 0) &&
            !queuePop(push->videoQueue, (GHandle *) &packet, GINT32_MAX)) {
            if (push->stopRequest) {
                break;
            }
        }
...
GBool GJStreamPush_Create(GJStreamPush **sender, MessageHandle callback, void *streamPushParm, const GJAudioStreamFormat *audioFormat, const GJVideoStreamFormat *videoFormat) {
    ...
    av_register_all();
    queueCreate(&push->videoQueue, 200, GTrue, GTrue);
    queueCreate(&push->audioQueue, 400, GTrue, GTrue);
...
GBool GJStreamPush_StartConnect(GJStreamPush *push, const char *sendUrl) {
    ...
    push->stopRequest = GFalse;
    push->startMS     = 0;
    queueEnablePush(push->videoQueue, GTrue);
    queueEnablePop(push->videoQueue, GTrue);
    queueEnablePush(push->audioQueue, GTrue);
    queueEnablePop(push->audioQueue, GTrue);
    char *format = GNULL;
    char  preUrl[5];
...
GVoid GJStreamPush_Delloc(GJStreamPush *push) {
    ...
    queueFuncClean(push->videoQueue, R_BufferUnRetainUnTrack);
    queueFree(&push->videoQueue);
    queueFuncClean(push->audioQueue, R_BufferUnRetainUnTrack);
    queueFree(&push->audioQueue);
...
GVoid GJStreamPush_Close(GJStreamPush *sender) {
    if (sender->stopRequest) {
        ...
        queueEnablePush(sender->videoQueue, GFalse);
        queueEnablePop(sender->videoQueue, GFalse);
        queueEnablePush(sender->audioQueue, GFalse);
        queueEnablePop(sender->audioQueue, GFalse);
    }
}
...
GBool GJStreamPush_SendVideoData(GJStreamPush *push, R_GJPacket *packet) {
    ...
    if (queuePush(push->videoQueue, packet, 0)) {
...
GBool GJStreamPush_SendAudioData(GJStreamPush *push, R_GJPacket *packet) {
    ...
    R_BufferRetain(packet);
    if (queuePush(push->audioQueue, packet, 0)) {

//        queueBroadcastPop(push->videoQueue);
