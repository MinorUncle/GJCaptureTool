...
-(BOOL)start:(NSError**)error recoveringFromErrors:(BOOL)recoverFromErrors {
    ...
    if ( ![[GJAudioSessionCenter shareSession]activeSession:YES key:[NSString stringWithFormat:@"%p",self] error:&error] ) {
        return NO;
    }
...
- (void)stopInternal {
    ...
    if ( !_interrupted ) {
        ...
        if ( ![[GJAudioSessionCenter shareSession]activeSession:NO key:[NSString stringWithFormat:@"%p",self] error:&error] ) {
...
-(void)setAudioSessionCategory:(NSString *)audioSessionCategory {
    ...
    if ( [_audioSessionCategory isEqualToString:AVAudioSessionCategoryRecord] || [_audioSessionCategory isEqualToString:AVAudioSessionCategoryPlayAndRecord]) {
        if ( ![[GJAudioSessionCenter shareSession]requestRecode:YES key:[NSString stringWithFormat:@"%p",self] error:&error] ) {
            NSLog(@"TAAE: Error setting audio session category: %@", error);
        }
    }else{
        if ( ![[GJAudioSessionCenter shareSession]requestRecode:NO key:[NSString stringWithFormat:@"%p",self] error:&error] ) {
            NSLog(@"TAAE: Error setting audio session category: %@", error);
        }
    }
    if (_allowMixingWithOtherApps) {
        if(![[GJAudioSessionCenter shareSession]requestMix:YES key:[NSString stringWithFormat:@"%p",self] error:&error]){
            NSLog(@"TAAE: Error setting audio session category: %@", error);
        }
    }else{
        if(![[GJAudioSessionCenter shareSession]requestMix:NO key:[NSString stringWithFormat:@"%p",self] error:&error]){
            NSLog(@"TAAE: Error setting audio session category: %@", error);
        }
    }
    
    if (_enableBluetoothInput) {
        if(![[GJAudioSessionCenter shareSession]requestBluetooth:YES key:[NSString stringWithFormat:@"%p",self] error:&error]){
            NSLog(@"TAAE: Error setting audio session category: %@", error);
        }
    }else{
        if(![[GJAudioSessionCenter shareSession]requestBluetooth:NO key:[NSString stringWithFormat:@"%p",self] error:&error]){
            NSLog(@"TAAE: Error setting audio session category: %@", error);
        }
    }
}
